...
class ViewController: NSViewController {
    ...
    private func setupVideoPreview() {
        ...
        view.layer?.addSublayer(previewLayer)
        previewLayer.frame = CGRect(x: 0.0, y: view.bounds.height - 180, width: 320, height: 180)
        previewLayer.connection?.videoRotationAngle = 0

        // Add points layer
        pointsLayer.frame = view.bounds
        view.layer?.addSublayer(pointsLayer)
        pointsLayer.fillColor = NSColor.red.cgColor
    }
    ...
    func updatePoints(_ points: [CGPoint]) {
        ...
        for point in convertedPoints {
            let dotRect = CGRect(x: point.x, y: point.y, width: 10, height: 10)
            let dotPath = CGPath(ellipseIn: dotRect, transform: nil)
            combinedPath.addPath(dotPath)
        }
