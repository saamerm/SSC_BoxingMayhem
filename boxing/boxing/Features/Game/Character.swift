...
#endif
#if canImport(AppKit)
            let image = NSImage(named: "\(prefix)-\(frameName)")
            if let image = image {
                var imageRect = CGRect(x: 0, y: 0, width: image.size.width, height: image.size.height)
                let imageRef = image.cgImage(forProposedRect: &imageRect, context: nil, hints: nil)

                let texture = SKTexture(cgImage: imageRef!)
                texture.filteringMode = .nearest
                if frameName == "none0" {
                    textures["none"] = [texture]
                } else {
...
#endif
#if canImport(AppKit)
            let image = NSImage(named:  "\(prefix)-\(state)")
            if let image = image {
                var imageRect = CGRect(x: 0, y: 0, width: image.size.width, height: image.size.height)
                let imageRef = image.cgImage(forProposedRect: &imageRect, context: nil, hints: nil)
...
#endif
#if canImport(AppKit)
            let image = NSImage(named:  "\(prefix)-knock\(i)")
            if let image = image {
                var imageRect = CGRect(x: 0, y: 0, width: image.size.width, height: image.size.height)
                let imageRef = image.cgImage(forProposedRect: &imageRect, context: nil, hints: nil)
